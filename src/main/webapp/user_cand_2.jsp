		...
		<tr>
			...
			<td>
				<%					
					if(candBean.getApproveStatus() == 2)
					{%>
						<a href="change_status_2_servlet?id=<%=candBean.getId()%>&status=1" class="btn btn-success">Approve</a>	
					<%}else if(candBean.getApproveStatus() == 1)
					{%>
						<a href="change_status_2_servlet?id='<%=candBean.getId()%>'&status=2" class="btn btn-danger">Reject</a>
